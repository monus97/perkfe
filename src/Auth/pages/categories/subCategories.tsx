  ...
  const fetchAllCategories = async () => {
    try {
      ...
      if (response?.status == 200) {
        const filteredData = response?.data?.filter((item: any) => item?.isSubCategory == true);
      ...
      <table className="table mt-4 ">
        <thead style={{ backgroundColor: "red !important" }}>
          <tr className="table-light">
            ...
            <Colortext>
              {" "}
              <Typography className="wi">PARENT CATEGORIES</Typography>
            </Colortext>
            <Colortext>
              {" "}
        ...
        <tbody className="tableBody">
          {categoryList.map((category, index) => (
            <tr key={index}>
              ...
              <Colortext>
              {category?.parentcategory?.map((item: any, index: number) => (
